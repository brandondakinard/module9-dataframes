...
husky.games.2015[husky.games.2015$combined == max(husky.games.2015$combined), "uw_score"]
...
MostYardsScore <- function(games) {
  ...
  combined.yards <- rushing.yards + passing.yards
  games$combined <- combined.yards
  var <- games[husky.games.2015$combined == max(husky.games.2015$combined),]
  return(paste0("On ", var$date, ", ", var$passing_yards, " passing yards and ", var$rushing_yards, " rushing yards were scores for a combined total of ", var$combined, " yards "))
}
...
a.variable <- husky.games.2015[husky.games.2015$combined == max(husky.games.2015$combined),]
a.variable
