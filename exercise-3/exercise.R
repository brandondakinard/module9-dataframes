...
sum(data["Personal Care", "X1940"])

# How much money was spent on Food and Tobacco in 1960?
sum(data["Food and Tobacco", "X1960"])
...
DetectHighest <- function(year) {
  highest <- data["year" == max("year"), "category"]
  return(highest)
}

# Using your function, determine the highest spending category of each year
# method doesn't work correct :C
highest.one <- DetectHighest(X1940)
highest.two <- DetectHighest(X1945)
highest.three <- DetectHighest(X1950)
highest.four <- DetectHighest(X1955)
highest.five <- DetectHighest(X1960)